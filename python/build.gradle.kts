plugins {
    // 使用 m8test python gradle 插件, 表示这是一个 m8test python 项目
    alias(m8test.plugins.m8test.gradle.script.python)
}
...
//    properties["PYTHON_PATH"] = "tools/python3" // Python 可执行文件路径
//    properties["PIP_PATH"] = "tools/pip3" // pip 可执行文件路径
    ...
    settingsConfig {
        language {
            properties["com.m8test.extension.id"] = "com.m8test.extension.language.python"
            version = languagePluginVersion
            url =
                "https://github.com/m8test/language-release/releases/download/python-\$version/com.m8test.python-release_\$version.apk"
        }
    }
    // 脚本项目配置闭包
    projectConfig {
...
//        }
    ...
    tools {
        ...
        test {
            ...
            paths["build/test/test.py"] = "/sdcard/M8Test/test.py"
            paths["build/test/test1.py"] = "/sdcard/M8Test/test1.py"
        }
