...
本分支已完成 Lua Extension 适配当前 `m8test-android`，Android 端由 `luakt` 执行器负责运行和 JVM 桥接，因此本轮同步迁移统一 Logger 并验证 `luakt` 边界。
...
- 设备验收执行模板的 `runLua`/`rerunLua` 任务，确认 BuildProjectSource、BuildScript、luakt 主脚本、side 脚本和日志输出。
